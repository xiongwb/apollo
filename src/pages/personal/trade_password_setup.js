...
class TradePasswordSetup extends BasePage {
   constructor(props){
    super(props);
      this.state = {
        disabled:true,
    ...
    ApolloAPI.APILogin.getloginState({
      tenantNo:COMMON_CONFIG.tenantNo
        ...
        if(res_json.retCode == 1){
          ...
          let hash = JSON.parse(value);
          if(!!hash && hash.retCode == 1 && hash.retMsg){
            if(this.props.validate){//从交易密码进来
                        ...
                        if(res_json.retCode == 1){
                          if(this.state.password===this.state.confirmPassword){
                            ApolloAPI.APIPersonal.updateTranPswd({
                              ...
                              this.get_loading().dismiss();
                              if(res_json.retCode == 1){
                                PlateFormUtils.plateFormAlert(Platform,'提示','设置交易密码成功！');
  ...
  render() {
    ...
    return (
      <View style={styles.root}>
        {this.props.validate?
          <BackNavBar style={{backgroundColor:'#ed5565'}} component={this}>设置交易密码</BackNavBar>
          :
          <NavBar 
            backgroundColor='#ed5565'
            titleContent={<Text style={{color:'#fff',fontSize:18}}>设置交易密码</Text>} 
            />
        }
        <View style={{flex:1}}>
...
export default TradePasswordSetup