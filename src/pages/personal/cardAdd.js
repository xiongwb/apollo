 ...
 *2017-4-14
 */
class CardAdd extends BasePage {
    constructor(props) {
        super(props)
        this.state = {
            regionName: '',
   ...
   async request_api() {
    ...
    _onsubmit = async () => {
            ...
            if (this.state.onChecked == false) {
                ...
                ApolloAPI.APIcard.saveBindBank({//银行卡管理，添加银行卡=提现和充值添加银行卡
                     telno :this.state.telno,//银行卡号---
                    ...
                    this.get_loading().dismiss();
                    if (res_json.retCode == 1) {
                        RCTDeviceEventEmitter.emit(EVENT_EMITTER_CONST.DIDBNDCARDSUCCESS, 'bindcard'); 
                        PlateFormUtils.plateFormAlert(Platform, '提示', '绑卡成功！');
    ...
    render() {
        return (
            <View style={styles.root}>
                <BackNavBar style={{backgroundColor: '#ed5565'}} component={this}>添加银行卡</BackNavBar>
                <ScrollView style={styles.excelContainer} showsVerticalScrollIndicator={false}>
                    <FormProvider form={this.form}>
                        <View>
                            <MgAddCardtxt name='cusname' placeholder={this.state.cusname} bBWidthdraw={1} flexdraw={1.3} colorTex='#101010' editable={false} disabled={true}>客户名称</MgAddCardtxt>
                            ...
                            <MgAddCardtxt name="certno" placeholder={this.state.certno} bBWidthdraw={1} flexdraw={1.3} colorTex='#101010' editable={false} disabled={true}>证件号码</MgAddCardtxt>
                            <View  style={{justifyContent:'center', borderBottomColor:'#e5e5e5',borderBottomWidth:1,flexDirection:'row',flex:1,borderColor:'#e5e5e5',height:44}}>
                                <View style={{ justifyContent: 'center', marginLeft:12,flex:1.08}}>
                            ...
                            value={this.state.bankname!=null?this.state.bankname:null||this.state.bankname} 
                            _rightButtonClick={() => this.props.navigator.push({id: "OpenBank",params: {openbank:this}})} 
                            ImageRight={require('apollo/src/image/arrow_right.png')} 
                            keyboardType="numeric" bBWidthdraw={1}
                            placeholder='请点击选择开户行' 
                            flexdraw={1.3} colorTex='#101010' 
                            editable={false}>开户行</MgAddCardtxt>
                            ...
                            <View style={styles.btnContainer}>
                                <Submit onChecked={this.state.onChecked} haveVfcode={true} containerStyle={[styles.btn]} textStyle={[styles.textBtn]} onSubmit={this._onsubmit}>绑 定</Submit>                          
                            </View>
