 ...
 *2017-4-14
 */
class WaddCard extends BasePage {
    constructor(props) {
        super(props)
        this.state = {
            regionName: '',
    ...
    _onsubmit = async () => {
            ...
            if (this.state.onChecked == false) {
                ...
                ApolloAPI.APIcard.saveBindBank({//银行卡管理，添加银行卡=提现和充值添加银行卡
                    ...
                    this.get_loading().dismiss();
                    if (res_json.retCode == 1) {
                        RCTDeviceEventEmitter.emit(EVENT_EMITTER_CONST.BANKINFOSUCCESS, 'bindcard'); 
            ...
            <View style={styles.root}>
                ...
                <ScrollView style={styles.excelContainer} showsVerticalScrollIndicator={false}>
                    <FormProvider form={this.form}>
                        <View>
                            <MgAddCardtxt name='cusname' value={this.state.cusname} bBWidthdraw={1} flexdraw={1.3} colorTex='#101010' editable={false}>客户名称</MgAddCardtxt>
                            ...
                            <MgAddCardtxt name="certno" value={this.state.certno} bBWidthdraw={1} flexdraw={1.3} colorTex='#101010' editable={false}>证件号码</MgAddCardtxt>
                            <View  style={{justifyContent:'center', borderBottomColor:'#e5e5e5',borderBottomWidth:1,flexDirection:'row',flex:1,borderColor:'#e5e5e5',height:44}}>
                                <View style={{ justifyContent: 'center', marginLeft:12,flex:1.08}}>
